---
title: "Digital Instrument"
layout: single
permalink: /digital-instrument/
author_profile: true
header:
  overlay_image: /assets/images/headers/instrument-header.jpg
  overlay_filter: "0.3"
  image_description: "The Chemnitzer held in a playing position"
---
The Chemnitzer's unique, rich sound is now available as a Sforzando digital instrument - download below and try it out for yourself!  The .SFZ file can be loaded as a MIDI instrument by the free Plogue Sforzando player on any platform.

<a href="https://github.com/jTomiak-28/tomiak-chemnitzer.github.io/releases/download/v1.0/Chemnitzer.zip" download
   style="
     display: inline-block;
     padding: 0.6em 1.2em;
     font-size: 1rem;
     color: #fff;
     background-color: #007acc;
     border-radius: 5px;
     text-decoration: none;
   ">
   Download My Instrument
</a>

The Chemnitzer is capable of playing most notes between E2 and F#6. To produce this digital instrument, I sampled each of the available pitches in this range at three dynamic levels, plus a release sample.  I added some light sound control and normalization, set loop points, and configured the .SFZ file to allow Plogue to find all the proper samples. The result is a digital instrument that works naturally in a MIDI keyboard layout.

The whole .SFZ design is open-source, so the .zip above contains all the .wav samples plus the .SFZ configuration file. I'm releasing this digital instrument under the permissive CC0 license, so users are encouraged to tinker with the configuration and apply this resource however they see fit, no strings attached.

The Chemnitzer was recorded with two AKG P170s in close ORTF plus a medium-distance LDC lightly mixed in.  I left a small stereo width to preserve the natural feel: bass on the right side, treble on the left.
